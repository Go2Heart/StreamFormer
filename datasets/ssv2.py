...
class SSRawFrameClsDataset(Dataset):
    ...
    def copy_dataset(self, num_copies):
        if isinstance(self.data_list, pd.DataFrame):
            self.dataset_samples = pd.concat([self.dataset_samples] * num_copies, ignore_index=True)
            self.label_array = pd.concat([self.label_array] * num_copies, ignore_index=True)
        else:
            self.dataset_samples = self.dataset_samples * num_copies
            self.label_array = self.label_array * num_copies
...
class SSVideoClsDataset(Dataset):
    ...
    def __init__(self, anno_path, prefix='', split=' ', mode='train', clip_len=8,
                ...
                num_crop=1, test_num_segment=10, test_num_crop=3, filename_tmpl=None, label2id_path=None,args=None):
        self.anno_path = anno_path
        self.prefix = prefix
        self.split = split
        self.mode = mode
            ...
            if self.mode == 'train':
                ...
                if args.num_sample > 1:
                    ...
                    if self.label2id is not None:
                        label_list = [self.label2id[label] for label in label_list]
                    return frame_list, label_list, index_list, {}
                else:
                    buffer = self._aug_frame(buffer, args)
                
                buffer = buffer.permute(1, 0, 2, 3) # T C H W # check shape TODO
                if self.label2id is not None:
                    return {
                        'task_name': 'SSV2',
                        'task_input': {
                            'video': buffer,
                            'label': self.label2id[self.label_array[index]],
                            'video_id': self.dataset_samples[index],
                            'caption': "RESERVED"
                        }
                    } # TODO:debug for now, merged in the multi-task dataset
                    return buffer, self.label2id[self.label_array[index]], index, {}
                return buffer, self.label_array[index], index, {}
                ...
                if self.label2id is not None:
                    ...
                    return buffer, self.label2id[self.label_array[index]], sample.split("/")[-1].split(".")[0]
                ...
                buffer = buffer.permute(1, 0, 2, 3) # T C H W
                if self.label2id is not None:
                    return buffer, self.label2id[self.test_label_array[index]], sample.split("/")[-1].split(".")[0], \
                    chunk_nb, split_nb
                return buffer, self.test_label_array[index], sample.split("/")[-1].split(".")[0], \
                    chunk_nb, split_nb
            else:
