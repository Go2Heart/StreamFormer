...
class TaskVISDataset(Dataset):
    ...
    def __init__(
        ...
        for per_data_anno in os.listdir(self.anno_path):
            if not (
                per_data_anno.startswith("train_") and per_data_anno.endswith(".json")
            ):
                continue
            ...
            print(
                f"After processing dataset: {cur_dataset}, there are {len(videos)} videos and {len(annotations)} annotations"
            )
        ...
        for anno in annotations:
            ...
            if anno["iscrowd"] and anno["segmentation"] is not None:
                ...
                if anno["segmentation"] is not None:
                    masks.append(
                        self.polygons_to_mask(image_shape, anno["segmentation"])
                    )
                else:
    ...
    def process_youtube_vis(self, video, annotations, dataset_name):
        ...
        buffer, all_index = self.load_frame(
            frame_names, dataset_name, total_frame
        )  # T H W C
        ...
        for anno in annotations:
            ...
            for idx in all_index:
                if anno["segmentations"][idx] is not None:
                    mask = pycocotools.mask.frPyObjects(
                        anno["segmentations"][idx], anno["height"], anno["width"]
                    )
                    mask = pycocotools.mask.decode(mask)
                    mask = np.array(mask, dtype=np.uint8)
                    masks.append(mask)
                    ...
                    masks.append(
                        np.zeros((anno["height"], anno["width"]), dtype=np.uint8)
                    )
        ...
        for category_id, masks in anno_masks.items():
            mask_target[masks != 0] = (
                category_id  # [T,H,W] with category_id indicating the label
            )
    ...
    def __getitem__(self, index):
        try:
            ...
            if dataset_name == "COCOPseudoVIS":
                buffer, mask_target, masks = self.process_coco_pseudo_vis(
                    video, annotations, dataset_name
                )
            elif dataset_name == "LVVIS":
                buffer, mask_target, masks = self.process_lvvis(
                    video, annotations, dataset_name
                )
            elif dataset_name == "YoutubeVIS":
                buffer, mask_target, masks = self.process_youtube_vis(
                    video, annotations, dataset_name
                )
            else:
            ...
            ), f"<task_{dataset_name}> mask_target is None or No foreground"
    ...
    def load_frame(self, sample, dataset_name, num_frames, sample_rate_scale=1):
        ...
        frame_names = [
            os.path.join(self.prefix_dict[dataset_name], fname) for fname in sample
        ]
