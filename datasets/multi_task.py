...
class MultiTaskDataset(Dataset):
    ...
    def __init__(
        self, datasets_dict, balance_sample_num=True, batch_size=16, scale=2.0
    ):
        ...
        self.unified_dataset = tuple(
            datasets_dict.values()
        )  # tuple is more memory efficient than list
        self.unified_dataset_lengths = tuple(
            len(dataset) for dataset in self.unified_dataset
        )
        self.cumulative_lengths = tuple(
            [0] + list(accumulate(self.unified_dataset_lengths))
        )
        self.task_names = tuple(datasets_dict.keys())
        task_info = {
            task_name: len(dataset) for task_name, dataset in datasets_dict.items()
        }
    ...
    def __getitem__(self, idx):
        ...
        if dataset_idx == -1:
            print(
                idx, self.cumulative_lengths
            )  # fake batch dataset has to be appended at the end for now
