...
class ReferYoutubeVOSDataset(Dataset):
    ...
    def __getitem__(self, index):
        try:
            ...
            assert (
                mask_target.sum() > 0
            ), "mask_target does not exist for index: {}, video_id: {}, text: {}".format(
                index, video_id, text
            )
    ...
    def load_video_and_mask(self, sample, frame_names, obj_id, total_frame):
        ...
        for j in range(self.num_segment):
            ...
            img_path = os.path.join(
                str(self.prefix), "JPEGImages", sample, frame_name + ".jpg"
            )
            mask_path = os.path.join(
                str(self.prefix), "Annotations", sample, frame_name + ".png"
            )
