...
import random 
import pandas as pd 
from utils_ret import RawVideoExtractor, read_frames_decord
...
# from utils_ret import get_transform
...
class TaskRetrievalDataset(Dataset):
    ...
    def __init__(
        self, anno_path, prefix='', mode='train',
        clip_len=8, num_segment=1, num_crop=1, 
        ...
        self.transform = self.get_transform(
            mode=self.mode, init_model='siglip', 
            image_res=self.crop_size, random_aug=False,
        )
        ...
        for retrieval_dataset in ['MSRVTT', 'MSVD', 'ActivityNet', 'DiDeMo', 'LSMDC', 'VATEX', 'WebVid']:
            if retrieval_dataset not in self.params_dict['rootdir']:
                continue
            print(f'{retrieval_dataset} rootdir: {self.params_dict["rootdir"][retrieval_dataset]}, trimmed30s: {self.params_dict["trimmed30s"][retrieval_dataset]}, is_paragraph_retrieval: {self.params_dict["is_paragraph_retrieval"][retrieval_dataset]}')
