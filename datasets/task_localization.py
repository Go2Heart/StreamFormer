...
class TaskLocalizationDataset(Dataset):
    ...
    def __init__(self, anno_path, prefix='', mode='train', clip_len=8,
                 ...
                 label2id_path=None, sample_type='uniform',
                 data_dict=None,
                 args=None):
    ...
    def __getitem__(self, index):
        try:
            video_path, start_time, end_time, text, dataset, relevant_windows, label = self.get_video_info(index)
            
            if self.sample_type == 'fixfps':
                buffer, label, duration, timestamps = self.loadvideo_decord_fixfps(video_path, start_time, end_time, self.clip_len, relevant_windows, label)
            else:
                buffer, label, duration, timestamps = self.loadvideo_decord(video_path, start_time, end_time, self.clip_len, relevant_windows, label)
    ...
    def loadvideo_decord_fixfps(self, sample, start_time, end_time, clip_len, relevant_windows, label):
        ...
        if len(relevant_windows) <= 1:
            labels = torch.tensor([label if start_time <= time <= end_time else 0 for time in timestamps], dtype=torch.float32)
        else:
    ...
    def loadvideo_decord(self, sample, start_time, end_time, clip_len, relevant_windows, label):
        """Load video content using Decord, calculate duration, and generate time labels based on frame indices."""
        fname = sample
        ...
        if len(relevant_windows) <= 1:
            labels = torch.tensor([label if start_time <= time <= end_time else -1 for time in timestamps], dtype=torch.float32)
        else:
