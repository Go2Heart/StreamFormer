...
class TaskReferVOSDataset(Dataset):
    ...
    def __init__(self, anno_path, prefix='', split=' ', mode='train',
                 ...
                 test_num_crop=3, filename_tmpl='img_{:05}.jpg', data_dict=None, args=None):
        ...
        if data_dict is not None:
            ...
            self.mask_root_dict = {
                ...
                "ReferYoutubeVOS": os.path.join(os.path.dirname(self.prefix_dict["ReferYoutubeVOS"]), "Annotations"),
        ...
        for per_data_anno in os.listdir(self.anno_path):
            ...
            print(f'After processing dataset: {cur_dataset}, there are {len(data)} videos')
    ...
    def process_mevis(self, video):
        ...
        buffer, all_index = self.loadvideo_decord(video_id + '.mp4', total_frame)  # T H W C
        ...
        assert mask_target is not None and mask_target.sum() > 0, '<task mevis> mask_target is None or No foreground'
    ...
    def process_refcoco_pseudo(self, video, annotation, dataset_name):
        ...
        assert mask_target.sum() > 0, '<task refcoco pseudo> mask_target has no foreground'
    ...
    def __getitem__(self, index):
        try:
            ...
            if dataset_name == 'MEVIS':
                ...
                buffer, mask_target, text = self.process_refcoco_pseudo(video, annotation, dataset_name)
            
            assert mask_target is not None and mask_target.sum() > 0, f'{dataset_name} mask_target has no foreground'
    ...
    def load_video_and_mask(self, sample, frame_names, obj_id, total_frame):
        ...
        for j in range(self.num_segment):
            ...
            img_path = os.path.join(self.prefix_dict['ReferYoutubeVOS'], sample, frame_name + '.jpg')
            mask_path = os.path.join(self.mask_root_dict['ReferYoutubeVOS'], sample, frame_name + '.png')
