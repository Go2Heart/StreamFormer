...
class MetricLogger(object):
    ...
    def log_every_joint(
        self, video_loader, image_loader, print_freq, header=None, image_num_ratio=1.0
    ):
...
def save_on_master(obj, ckpt_path, ceph_args):
    if is_main_process():
        if ceph_args["use_ceph_checkpoint"]:
            ...
            print("Save to ceph_ckpt_path:", ceph_ckpt_path)
...
def init_distributed_mode(args):
    if args.dist_on_itp:
        ...
        master_port = os.environ.get("MASTER_PORT", "29482")
        ...
        args.local_rank = args.gpu = int(proc_id % num_gpus)
        print(
            f"SLURM MODE: proc_id: {proc_id}, ntasks: {ntasks}, node_list: {node_list}, num_gpus:{num_gpus}, addr:{addr}, master port:{master_port}"
        )
...
def save_model(
    ...
    if loss_scaler is not None:
        ...
        model.save_checkpoint(
            save_dir=local_save_dir, tag=tag_name, client_state=client_state
        )
...
def auto_load_model(
    ...
    if ceph_args["use_ceph_checkpoint"]:
        ...
        if loss_scaler is not None:
            ...
            elif check_ceph_exists(
                os.path.join(ceph_save_dir, "checkpoint-latest.pth")
            ):
                args.resume = os.path.join(ceph_save_dir, "checkpoint-latest.pth")
            elif args.auto_resume and len(args.resume) == 0:
                all_checkpoints = fnmatch.filter(
                    list(client.list(ceph_save_dir)), "checkpoint-*"
                )
                all_checkpoints = [
        ...
        if loss_scaler is not None:
            ...
            elif os.path.exists(os.path.join(output_dir, "checkpoint-epoch.pth")):
                args.resume = os.path.join(output_dir, "checkpoint-epoch.pth")
            ...
            if args.resume:
                ...
                print("Resume checkpoint %s" % args.resume)
                res = model_without_ddp.load_state_dict(
            ...
            if args.test_best and args.eval:
                try:
                    load_specific_model(
                        model, model_ema, args, output_dir, model_name="best"
                    )
                    flag = True
                except Exception:
                    print("No best model")
            if not flag:
                try:
                    load_specific_model(
                        model, model_ema, args, output_dir, model_name="latest"
                    )
                    flag = True
                except Exception:
                    print("No latest model")
            ...
            if not flag:
                ...
                if latest_ckpt >= 0:
                    load_specific_model(
                        model, model_ema, args, output_dir, model_name=latest_ckpt
                    )
                else:
                    print("No other models")
...
def load_specific_model(model, model_ema, args, output_dir, model_name):
    ...
    _, client_states = model.load_checkpoint(
        args.output_dir, tag=f"checkpoint-{model_name}"
    )
...
def load_specific_ceph_model(
    model, model_ema, args, sub_path, ceph_save_dir, model_name, ceph_args
):
    ...
    _, client_states = model.load_checkpoint(
        os.path.join("/dev/shm", sub_path), tag=f"checkpoint-{model_name}"
    )
    args.start_epoch = client_states["epoch"] + 1
    if model_ema is not None:
        if args.model_ema:
...
def construct_task_input_template(dataset_train):
    task_input_template = {}
    unique_keys = {}
    for sample_idx in dataset_train.cumulative_lengths[:-1]:
        sample_task = dataset_train[sample_idx]
        task_name = sample_task["task_name"]
        task_input = sample_task["task_input"]
        current_keys = task_input.keys()

        # unique_keys.update(current_keys)
        for key, value in task_input.items():
            if isinstance(value, torch.Tensor):
                template = torch.zeros_like(value)
            elif isinstance(value, np.ndarray):
                template = np.zeros_like(value)
            elif isinstance(value, str):
                template = ""
            elif isinstance(value, int):
                template = torch.LongTensor([0])
            elif isinstance(value, float):
                template = torch.FloatTensor([0.0])
                ...
                print("Need to add type: ", type(value))

            if key not in task_input_template:
                task_input_template[key] = template

    return task_input_template


def multiple_tasks_samples_collate(batch, task_input_template):
    """
    Collate function for repeated augmentation. Each instance in the batch has
    more than one sample.
    Args:
    ...
    Returns:
        (tuple): collated data batch.
    """
    all_task_keys = task_input_template.keys()

    all_task_names = []
    all_task_inputs = {}
    for each_key in all_task_keys:
        all_task_inputs[each_key] = []

    for sample_idx, per_sample in enumerate(batch):
        task_name = per_sample["task_name"]
        task_input = per_sample["task_input"]

        full_task_input = task_input_template.copy()
        full_task_input.update(task_input)

        all_task_names.append(task_name)
        for each_key in all_task_keys:
            all_task_inputs[each_key].append(full_task_input[each_key])

    for each_key in all_task_keys:
        # if each_key == 'gt_segments' or each_key == 'gt_labels' or each_key == 'masks' or (each_key == 'video' and task_name == "THUMOS14"):
        if task_name in ["THUMOS14", "ActivityNet", "FineAction", "HACS"]:
            if each_key == "video":
                all_task_inputs[each_key] = default_collate(
                    [all_task_inputs[each_key][0]]
                )  # fake batch size, use only one sample
            else:
                pass  # due to the different length of gt_segments, gt_labels, masks, we do not collate them, and let them be a list to be handled in the model side
        elif task_name in ["YoutubeVIS", "LVVIS", "MEVIS", "COCOPseudoVIS", "TaskVIS"]:
            if each_key == "video":
                all_task_inputs[each_key] = default_collate(
                    all_task_inputs[each_key]
                )  # only collate video, other keys are not needed to be collated
            else:
                pass
        else:
            all_task_inputs[each_key] = default_collate(all_task_inputs[each_key])
    multi_task_batch_data = {"task_name": all_task_names, "task_input": all_task_inputs}
    return multi_task_batch_data
...
def collate_neglect_dict(batch):
    elems = batch[0]
    collated_data = []
    for idx, elem in enumerate(elems):
        if isinstance(elem, dict):
            # Retain dict keys and collate values across the batch
            new_dict = {}
            for key in elem.keys():
                new_dict[key] = [sample[idx][key] for sample in batch]
            collated_data.append(new_dict)
        else:
            try:
                tensors = [
                    (
                        torch.tensor(sample[idx])
                        if isinstance(sample[idx], np.ndarray)
                        else sample[idx]
                    )
                    for sample in batch
                ]
                collated_data.append(default_collate(tensors))
                # collated_data.append(default_collate([sample[idx] for sample in batch]))
            except Exception as e:
                # Detailed error information
                print(f"Error collating tensors at index {idx}: {e}")
                for sample in batch:
                    print(f"Shape: {sample[idx].shape}, Dtype: {sample[idx].dtype}")
                raise
