...
def multiple_tasks_samples_collate(batch, task_input_template):
    ...
    for each_key in all_task_keys:
        ...
        if task_name in ["THUMOS14", "ActivityNet", "FineAction", "HACS"]:
            if each_key == 'video':
                all_task_inputs[each_key] = default_collate([all_task_inputs[each_key][0]]) #fake batch size, use only one sample
            else:
            ...
            if each_key == 'video':
                all_task_inputs[each_key] = default_collate(all_task_inputs[each_key]) # only collate video, other keys are not needed to be collated
            else:
                pass
        else:
            all_task_inputs[each_key] = default_collate(all_task_inputs[each_key])
    multi_task_batch_data = {
        'task_name': all_task_names,
        'task_input': all_task_inputs
    }
...
def collate_neglect_dict(batch):
    ...
    for idx, elem in enumerate(elems):
        if isinstance(elem, dict):
            ...
            try:
                tensors = [torch.tensor(sample[idx]) if isinstance(sample[idx], np.ndarray) else sample[idx] for sample in batch]
