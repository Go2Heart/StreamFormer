    ...
    def forward(self, pixel_values):
        ...
        if self.attention_type != "space_only":
            ...
            if num_frames != self.time_embeddings.size(1):
                if num_frames < self.time_embeddings.size(1):
                    new_time_embeddings = self.time_embeddings[:, :num_frames, :]  # (1, num_frames, D)
                else:
...
class TimesformerMultiTaskingModelSigLIP(TimesformerPreTrainedModel):
    ...
    def forward(
        ...
        sequence_output = sequence_output.reshape(B, -1, T, sequence_output.size(-1)).permute(0, 2, 1, 3).reshape(B, T, -1, sequence_output.size(-1)) # (B, T, N, D)
        if not return_dict:
            return (sequence_output,) + encoder_outputs[1:]
        return BaseModelOutputWithPooling(
            last_hidden_state=sequence_output,
    ...
    def forward(self, task_head_input: ModelOutput, task_specific_input: Optional[dict] = None) -> torch.Tensor:
        ...
        for i in range(dense_image_embeds.shape[0]):
            ...
            if curr_text_embeds.shape[0] > NUM_MAX_CLASSES:
                ...
                new_mask_target = new_mask_target.masked_fill(new_mask_target == 0, -1) # ignore background
            ...
            curr_logits = curr_logits.reshape(curr_logits.shape[0], patch_num, patch_num, curr_logits.shape[2])
...
class TimesformerTemporalGroundingHead(nn.Module):
    ...
    def forward(self, task_head_input: ModelOutput, task_specific_input: Optional[dict] = None) -> torch.Tensor:
        ...
        input_ids = self.text_tokenizer(task_specific_input['caption'], return_tensors="pt", padding="max_length", truncation=True, max_length=64).to(image_embeds.device)
        outputs = self.text_encoder(**input_ids)
