...
class InteractionBlock(nn.Module):
    ...
    def forward(self, x, c, blocks, deform_inputs1, deform_inputs2, H, W, T):
        ...
        x = x.reshape(x.shape[0], -1, T, x.size(-1)).permute(0, 2, 1, 3).reshape(x.shape[0]*T, -1, x.size(-1))
...
class TimesformerMultiTaskingModelSigLIPViTAdapter(TimesformerPreTrainedModel):
    ...
    def __init__(
        ...
        self.norm_layer = partial(nn.LayerNorm, eps=config.layer_norm_eps) # TODO check if this is correct
    ...
    def forward(
        ...
        for i, layer in enumerate(self.interactions):
            ...
            x, c = layer(x, c, self.encoder.layer[indexes[0]:indexes[-1] + 1], deform_inputs1, deform_inputs2, H, W, T)
            ...
            x = x.reshape(B, T, -1, embed_dim).permute(0, 2, 1, 3).reshape(B, -1, embed_dim)
