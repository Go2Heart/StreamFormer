...
from tools.finetune_tools import train_one_epoch_multi_task, validation_one_epoch_multi_task
import utils
from utils import NativeScalerWithGradNormCount as NativeScaler
from utils import multiple_samples_collate, multiple_tasks_samples_collate, construct_task_input_template
from sampler import DistributedBatchTaskUniqueSampler, DistributedBatchTaskSequentialSampler, DistributedBatchTaskBalancedSampler
...
def get_args():
    ...
    parser.add_argument("--prefix", type=str, default=None, help="Prefix for the dataset")
    ...
    # Model
    parser.add_argument("--init_vit", type=str, choices=["clip", "siglip", "sam", "siglip_sam"], default="clip")
    parser.add_argument("--enable_causal_temporal", action="store_true") # TODO modify to the model, currently stored in the model's config
    ...
    # Finetuning
    parser.add_argument("--sampler_type", type=str, default="unique", choices=["balanced", "unique"])
    ...
    parser.add_argument("--aa", type=str, default="rand-m7-n4-mstd0.5-inc1", help="Augmentation type")
    ...
    parser.add_argument("--sample_type", default='uniform', choices=['uniform', 'fixfps'])
...
def main(args, ds_init):
    ...
    dataset_train, dataset_eval, multi_task_config = build_multi_task_dataset(dataset_metadata, args)

    if args.enable_multitask_collate:
        task_input_template = construct_task_input_template(dataset_train) 
        collate_func = partial(multiple_tasks_samples_collate, task_input_template=task_input_template)
    ...
    if args.sampler_type == "balanced":
        sampler_train = DistributedBatchTaskBalancedSampler(dataset_train, args.batch_size, num_replicas=num_tasks, rank=global_rank, shuffle=True)
    elif args.sampler_type == "unique":
        sampler_train = DistributedBatchTaskUniqueSampler(dataset_train, args.batch_size, num_replicas=num_tasks, rank=global_rank, shuffle=True)
    else:
        raise ValueError(f"Unknown sampler type: {args.sampler_type}. Please choose from ['balanced', 'unique']")
    ...
    if args.init_vit == "clip":
        ...
        model = TimesformerForMultiTaskingSigLIP.from_pretrained(args.pretrained_model, multi_task_config, ignore_mismatched_sizes=True)
    ...
    print("Number of task heads parameters: {} | {:.2f}%".format(task_heads_parameters, 100 * task_heads_parameters/n_parameters))
    print("Trainable parameters:{} | {}%".format(sum(p.numel() for p in model.parameters() if p.requires_grad), 100 * sum(p.numel() for p in model.parameters() if p.requires_grad)/n_parameters))
    ...
    print("LR = {}, min_lr = {}, warmup_lr = {}".format(args.lr, args.min_lr, args.warmup_lr))
    ...
    if args.enable_deepspeed:
        ...
        if args.distributed:
            model = torch.nn.parallel.DistributedDataParallel(model, find_unused_parameters=True) # deviceids=[args.local_rank]
    ...
    for epoch in range(args.start_epoch, args.epochs):
        if args.distributed:
            ...
            data_loader_train.batch_sampler.set_epoch(epoch) # TODO check if this is correct
        if log_writer is not None:
            log_writer.set_step(epoch * num_training_steps_per_epoch * args.update_freq)
        train_stats = train_one_epoch_multi_task(model, criterion, data_loader_train, optimizer, device, epoch, loss_scaler, args.clip_grad, update_freq=args.update_freq, num_training_steps_per_epoch=num_training_steps_per_epoch, start_steps=0, log_writer=log_writer, bf16=args.bf16, lr_schedule_values=lr_schedule_values, wd_schedule_values=wd_schedule_values, multi_task_config=multi_task_config)
        if args.output_dir and args.save_ckpt and (epoch > 0 and epoch % 10 == 0 or epoch == args.epochs - 1) and utils.is_main_process():
            utils.save_model(
                args=args, model=model, model_without_ddp=model_without_ddp, optimizer=optimizer,
                loss_scaler=loss_scaler, epoch=epoch, model_name=f'epoch_{epoch}'
            )
        
        if args.output_dir and args.save_ckpt and utils.is_main_process():
            utils.save_model(
                args=args, model=model, model_without_ddp=model_without_ddp, optimizer=optimizer,
                loss_scaler=loss_scaler, epoch=epoch, model_name=f'epoch', ceph_args=ceph_args,
            )
            
        if data_loader_eval is not None:
            test_stats = validation_one_epoch_multi_task(data_loader_eval, model, device, multi_task_config, bf16=args.bf16, loc_eval=loc_eval)
            if 'acc1' in test_stats:
                ...
                if args.output_dir and args.save_ckpt:
                    utils.save_model(
                        args=args, model=model, model_without_ddp=model_without_ddp, optimizer=optimizer,
                        loss_scaler=loss_scaler, epoch=epoch, model_name='best', ceph_args=ceph_args,
                    )
            ...
            if log_writer is not None:
                ...
                if 'retrieval_top1' in test_stats:
                    log_writer.update(val_R_1=test_stats['retrieval_top1'], head="perf", step=epoch)
        ...
        if args.output_dir and utils.is_main_process():
            ...
            json_serializable_stats = {k: convert_to_json_serializable(v) for k, v in log_stats.items()}
