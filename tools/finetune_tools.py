...
def train_multi_task_batch(
    model, samples, multi_task_input, output_hidden_states=False
):
    loss, logits = model(
        pixel_values=samples,
        multi_task_input=multi_task_input,
...
@torch.no_grad()
def validation_one_epoch_retrieval(
    data_loader, model, device, ds=False, bf16=False, log_time=False
):
    ...
    for multi_task_input in metric_logger.log_every(data_loader, 10, header):
        ...
        if log_time:
            time0 = time.time()
        if log_time:
            print(f"Time to get batch: {time0 - time3}")
        ...
        if log_time:
            time1 = time.time()
        if log_time:
            print(f"Time to move data to GPU: {time1 - time0}")
        # compute output
        if ds:
            videos = videos.bfloat16() if bf16 else videos.half()
            batch_video_features, batch_text_features = model(
                pixel_values=videos, captions=captions
            )
        else:
            with torch.cuda.amp.autocast():
                batch_video_features, batch_text_features = model(
                    pixel_values=videos, captions=captions, inference=True
                )
...
def validation_one_epoch(
    ...
    for batch in metric_logger.log_every(data_loader, 10, header):
        ...
        if log_time:
            time2 = time.time()
        if log_time:
            print(f"Time to compute output: {time2 - time1}")
        ...
        if log_time:
            time3 = time.time()
        if log_time:
            print(f"Time to update meters: {time3 - time2}")
    ...
    for idx in range(prob.shape[0]):
        if prob[idx] > factor and idx not in vis_idx:
            ...
            while (
                ...
                and prob[end + 1] > factor
                and (end + 1 not in vis_idx)
            ):
                end += 1
                vis_idx[end] = 1
            timestamps_list.append(
                [timestamps[start], timestamps[end], 1]
            )  # add fake score '1' for now
...
@torch.no_grad()
def validation_one_epoch_grounding(
    data_loader, model, device, ds=False, bf16=False, log_time=False
):
        ...
        for factor in prob_factors:
            for idx in range(videos.shape[0]):
                ...
                if batch_id % 50 == 0 and idx == 0:
                    ...
                    print(
                        round(durations[idx].item(), 1),
                        [round(proposal[0].item(), 1), round(proposal[1].item(), 1)],
                        [gt_segment[0].item(), gt_segment[1].item()],
                        current_iou.item(),
                    )
                    print(results[factor], total_counts)

            metric_logger.meters[f"R1_3_{factor}"].update(
                results[factor][0.3] / total_counts, n=total_counts
            )
            metric_logger.meters[f"R1_5_{factor}"].update(
                results[factor][0.5] / total_counts, n=total_counts
            )
            metric_logger.meters[f"R1_7_{factor}"].update(
                results[factor][0.7] / total_counts, n=total_counts
            )
...
def train_one_epoch_multi_task(
    ...
    if loss_scaler is None:
        ...
        optimizer.zero_grad()
    for data_iter_step, multi_task_input in enumerate(
        ...
        if task_name == "SSV2" or task_name == "Kinetics":
            ...
            raise NotImplementedError(
                f"{task_name} is not supported in multi-task training"
            )
        ...
        if task_name == "SSV2" or task_name == "Kinetics":
            ...
            ground_truth = torch.arange(
                len(output), dtype=torch.long, device=output.device
            )
        ...
        if log_writer is not None:
            ...
            log_writer.update(
                **{f"{task_name}_loss_scale": loss_scale_value}, head="loss"
            )
            if task_name == "SSV2" or task_name == "Kinetics":
...
@torch.no_grad()
def validation_one_epoch_multi_task(
    data_loader,
    model,
    device,
    multi_task_config,
    ...
    avg_back_percent = 0
    back_count = 0
    for batch_id, multi_task_input in enumerate(
        ...
        with torch.cuda.amp.autocast(enabled=bf16):
            if task_name == "SSV2" or task_name == "Kinetics":
                loss, logits = model(videos, multi_task_input=multi_task_input)[
                    task_name
                ]
                ...
                batch_video_features, batch_text_features = model(
                    videos, multi_task_input=multi_task_input
                )[task_name]
                ...
                for factor in prob_factors:
                    for idx in range(videos.shape[0]):
                        ...
                        proposal = threshold_prob_proposal(
                            prob, timestamps[idx], factor=factor
                        )
                        ...
                        if batch_id % 50 == 0 and idx == 0:
                            ...
                            print(
                                round(durations[idx].item(), 1),
                                [
                                    round(proposal[0].item(), 1),
                                    round(proposal[1].item(), 1),
                                ],
                                [gt_segment[0].item(), gt_segment[1].item()],
                                current_iou.item(),
                            )
                ...
                for idx in range(videos.shape[0]):
                    probs = logits[
                        idx
                    ].sigmoid()  # if it's over 0.5, then it's a positive sample
                    proposed_segments = multi_segment_proposal(
                        probs, timestamps[idx], factor=0.5
                    )
                    jsonl_data.append(
                        {
                            ...
                            "pred_relevant_windows": [
                                ...
                                for start, end, score in proposed_segments
                            ],
    ...
    if "Kinetics" in multi_task_config.keys():
        print(
            "Kinetics: * Acc@1 {top1.global_avg:.3f} Acc@5 {top5.global_avg:.3f} loss {losses.global_avg:.3f}".format(
    ...
    if "MSRVTT" in multi_task_config.keys():
        ...
        metric_logger.meters["retrieval_top1"].update(
            retrieval_top1.item(), n=videos.shape[0]
        )
        print("text2video Recall@1: ", retrieval_top1)
        print(
            "MSRVTT: * Retrieval_top1 {ret_top1.global_avg:.3f}".format(
                ret_top1=metric_logger.retrieval_top1
            )
        )
    ...
    if "WebVid" in multi_task_config.keys():
        ...
        print("text2video Recall@1: ", retrieval_top1)
        print(
            "WebVid: * Retrieval_top1 {ret_top1.global_avg:.3f}".format(
                ret_top1=metric_logger.retrieval_top1
            )
        )
