...
def get_args():
    ...
    parser.add_argument('--data_set', default='Kinetics', choices=[
        ...
        'mitv1_sparse', 'SSV2_SIGLIP', 'Kinetics_SIGLIP'
    ...
    parser.add_argument('--enable_lora_spatial', action='store_true', default=True)
    parser.add_argument('--freeze_spatial', action='store_true', default=False)
    parser.add_argument('--ckpt_path', default='', type=str, help='path to the ckpt file')
...
def main(args, ds_init):
    ...
    for i in range(5):
        ...
        t = dataset_train[i]
        print('video:', t[0][0].shape, t[0][0].max(), t[0][0].min(), t[0][0].mean(), t[0][0].std(), 'label:', t[1][0])
        print('video:', t[0][1].shape, t[0][1].max(), t[0][1].min(), t[0][1].mean(), t[0][1].std(), 'label:', t[1][1])
    # for i in range(15):
    #     print(f'Loading sample {i}')
    #     t = dataset_test[i]
    #     print('video:', t[0].shape, t[0].max(), t[0].min(), t[0].mean(), t[0].std(), 'label:', t[1], 'chunk:', t[3], 'split:', t[4])
    #     breakpoint()
    ...
    model = TimesformerModelSigLIPForVideoClassification.from_pretrained(args.model, num_classes=args.nb_classes, ignore_mismatched_sizes=True)
    print(model)
    ...
    if args.ckpt_path:
        ...
        if 's3://' in args.ckpt_path:
            from petrel_client.client import Client
            client = Client()
            with io.BytesIO(client.get(args.ckpt_path)) as buffer:
        ...
        for k, v in ckpt['model'].items():
            if 'timesformer.' in k:
                newk = k.replace('timesformer.', '')
                new_ckpt[newk] = v
            elif 'text_encoder' in k or 'task_heads' in k:
                continue
            else:
                new_ckpt[k] = v
    ...
    patch_size = (model.config.patch_size, model.config.patch_size)
    ...
    if args.finetune:
        ...
        if 'pos_embed' in checkpoint_model:
            ...
            num_patches = model.config.patch_embed.num_patches # 
            num_extra_tokens = model.config.pos_embed.shape[-2] - num_patches # 0/1

            # we use 8 frames for pretraining
            orig_t_size = 8 // model.config.patch_embed.tubelet_size
            new_t_size = args.num_frames // model.config.patch_embed.tubelet_size
