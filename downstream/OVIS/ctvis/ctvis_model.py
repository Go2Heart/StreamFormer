...
class CTVISModel(MaskFormer):
    ...
    def from_config(cls, cfg):
        rets = TimesformerMaskFormer.from_config(cfg)  # noqa
    ...
    def train_model(self, batched_inputs):
        ...
        #     output_hidden_states=True,
        #     return_dict=True,
        # ).hidden_states  # L x (B, N*T, D)

        # _, num_frames, _, height, width = pixel_values.shape
        # num_patches = (height // self.patch_size) * (width // self.patch_size)
        # B, NT, D = features[0].shape
        # assert NT == num_patches * num_frames, f'NT: {NT}, num_patches: {num_patches}, num_frames: {num_frames}'

        # chosen_features = {}
        # for key, layer_index in self.chosen_layers.items():
        #     feat = features[layer_index]
        #     feat = feat.reshape(
        #         B, num_patches, num_frames, D  # (B, N, T, D)
        #         ).permute(0, 2, 3, 1  # (B, T, D, N)
